...
| 2026-04-10 | `docs/ai-pipeline.md` | Created: pipeline overview — stage breakdown, inputs/outputs table, assignment chunking and chunk instructions, stitching/dedup logic, section-level vs question-level patching, mermaid validation auto-fix loop, text-based flowchart, key source files table | S-093 | Agent-25 |
| 2026-04-10 | `docs/prompts.md` | Created: prompt template reference — variable system and full variable-to-source mapping, per-file docs for all 6 files in public/Prompts/ (content type, variables, consumers, structural rules), custom prompt template system (localStorage schema, CRUD API, injection into Creator system prompt) | S-094 | Agent-25 |
| 2026-04-10 | `src/lib/search-index.ts` | Created: inverted index with `buildIndex(items)` and `search(index, query)` — tokenizes title + topic + first 500 chars of markdown, ranks by hit count (exact=2, prefix=1), strips stopwords | S-096 | Agent-24 |
| 2026-04-10 | `src/app/content/page.tsx` | Replaced `searchContent` (.includes) with inverted index search; index built on mount + rebuilt on storage events via `subscribeToStorageChanges`; added recent searches (localStorage `news13n_recent_searches`, max 10, deduped) with chip dropdown on focus+empty and "Clear recent" link | S-096, S-097 | Agent-24 |
| 2026-04-10 | `src/components/GenerationForm.tsx` | S-045: Added per-field inline validation (topic 3-200 chars, questionCounts total>=3, customPrompt max 2000); errors shown on submit attempt with red border + message; submit button kept enabled. S-050: Active stepper label now has accent underline + accent text; pending labels muted to text-secondary/60. S-051: Normalized step-2 spacing to space-y-3, radio group gap-2, field groups space-y-1/space-y-2. | Agent-20 |
| 2026-04-10 | `src/components/FileUpload.tsx` | S-052: Replaced single `loading` boolean with per-file `FileState` (pending/processing/done/error); files processed sequentially; each row shows spinner/check/X icon; errors displayed inline per file below the row. | Agent-20 |
| 2026-04-10 | `src/app/page.tsx` | S-044: Added `MobileGenerationOverlay` component — fixed bottom, md:hidden, shows stage name, elapsed time, animated spinner, stage progress dots, positioned above mobile bottom nav with AnimatePresence. S-095: Added Stop button to `CompactGenerationStrip` (desktop, sm:inline-flex) and `MobileGenerationOverlay` (mobile, 44×44 touch target); wired to `abortRef.current?.abort()`; updated cancellation catch branch to save partial content and show toast | S-044, S-095 | Agent-22 |
...
| 2026-04-10 | `src/app/content/page.tsx` | Modified | S-043: Migrated filter/sort/viewMode persistence from sessionStorage to localStorage with keys news13n_lib_filter, news13n_lib_sort, news13n_lib_view | Agent-21 |
| 2026-04-10 | `src/components/content-viewer/ContentViewerHeader.tsx` | Modified | S-046: Added visible save status badge (Saved/Unsaved changes/Saving...) near title with color-coded styling; S-049: Added title attributes to back link and more-actions button | Agent-21 |
| 2026-04-10 | `src/app/settings/page.tsx` | Modified | S-048: Added tab navigation (Appearance/Prompts/Storage/About) with animated underline; each tab shows only its cards; active tab persisted to sessionStorage | Agent-21 |
| 2026-04-10 | `src/components/ExportMenu.tsx` | Modified | S-049: Added title attribute to export trigger button | Agent-21 |
| 2026-04-10 | `src/components/InlineAIPopover.tsx` | Modified | S-049: Added title attributes to close, cancel, undo, and replace buttons | Agent-21 |
| 2026-04-10 | `src/components/MarkdownEditor.tsx` | Modified | S-049: Added title attribute to undo chip dismiss button | Agent-21 |
